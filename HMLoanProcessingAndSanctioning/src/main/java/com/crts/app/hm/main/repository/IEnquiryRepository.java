...
public interface IEnquiryRepository extends CrudRepository<Enquiry, Integer>{
	...
	public int approvedUpdateStatusCode(@Param("statusCode")int statusCode,@Param("enquiryId")int enquiryId);
	
	@Modifying
	@Query("update Enquiry set statusCode= :statusCode where enquiryId= :enquiryId")
	public int pendingUpdateStatusCode(@Param("statusCode") int statusCode,@Param("enquiryId") int enquiryId);
}
